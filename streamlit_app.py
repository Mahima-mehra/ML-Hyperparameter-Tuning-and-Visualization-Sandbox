...
# To Disable Warnings
# NOTE: The "deprecation.showPyplotGlobalUse" config option was removed in
# modern Streamlit versions (it no longer exists and setting it raises an
# error). st.pyplot() now always requires an explicit figure to be passed in
# (which this app already does), so the option is no longer needed at all.
import warnings
...
def add_parameter_classifier_general(algorithm):
    ...
    if algorithm == "SVM":
        ...
        # NOTE: fixed a pre-existing typo of "poly " (with trailing space) which
        # caused sklearn's SVC to raise a ValueError when that option was selected.
    ...
    # Adding Parameters for Decision Tree
        ...
        # For this we will do a default random state till the user enters a
        # valid one, then it will be updated.
        # NOTE: st.sidebar.text_input always returns a string (or None), so
        # wrapping the *assignment* of params["random_state"] in try/except
        # is the correct pattern; this part already worked correctly.
    ...
    # Adding Parameters for Logistic Regression
        ...
        params["fit_intercept"] = fit_intercept == "True"
...
def choice_classifier(data, data_name, X, Y, ax):

    if data_name == "Diabetes":
        sc = ax.scatter(X[:, 0], X[:, 1], c=Y, cmap="viridis", alpha=0.8)
        ...
        colors = [
            ...
            "cyan", "pink", "magenta", "grey", "teal",
        ]
        sns.scatterplot(
            x=X[:, 0], y=X[:, 1], hue=Y,
            palette=sns.color_palette(colors), alpha=0.4, ax=ax,
        )
        ...
        colors = ["purple", "green"]
        sns.scatterplot(
            x=data["glucose"], y=data["bloodpressure"], data=data,
            hue=Y, palette=sns.color_palette(colors), alpha=0.4, ax=ax,
        )
...
def choice_regressor(X, x_test, predict, data, data_name, Y, ax):

    if data_name == "Diabetes":
        sc = ax.scatter(X[:, 0], Y, c=Y, cmap="viridis", alpha=0.4)
        ...
        sc = ax.scatter(X[:, 0], X[:, 1], cmap="viridis", c=Y, alpha=0.4)
        ax.plot(x_test, predict, color="red")
        ax.legend(["Actual Values", "Best Line or General formula"])
        ax.figure.colorbar(sc, ax=ax)
...
def data_model_description(algorithm, algorithm_type, data_name, data, X, Y):
    ...
    # Create figure and explicit axes — required by modern Streamlit so that
    # st.pyplot(fig) never touches the global pyplot state (thread-safe).
    fig, ax = plt.subplots()
    ...
    if algorithm_type == "Regressor":
        choice_regressor(X, x_test, predict, data, data_name, Y, ax)
    else:
        choice_classifier(data, data_name, X, Y, ax)
